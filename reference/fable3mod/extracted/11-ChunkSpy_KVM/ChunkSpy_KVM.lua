...
CONFIGURATION = {
  ["x86 standard"] = {
    ...
    size_size_t = 4,
    size_Instruction = 4,
    size_lua_Number = 4,        -- this & integral identifies the
    integral = 0,               -- type of lua_Number
    number_type = "single",     -- used for lookups
  },
  ["big endian int"] = {
...
config.FORMAT       = 2  -- LUAC_FORMAT (new in 5.1)
config.FPF          = 50 -- LFIELDS_PER_FLUSH
config.SIZE_OP      = 7  -- instruction field bits
config.SIZE_A       = 8
config.SIZE_B       = 8
config.SIZE_C       = 9
...
function DecodeInit()
  ...
  config.BITRK = math.ldexp(1, config.SIZE_C - 1)

  ---------------------------------------------------------------
  -- iABC instruction segment tables
  ---------------------------------------------------------------
  config.iABC = {       -- tables allows field sequence to be extracted
    config.SIZE_A,      -- using a loop; least significant field first
    config.SIZE_C,      -- additional lookups below, kludgy
    config.SIZE_B,
    config.SIZE_OP,
  }
  config.mABC = { MASK_A, MASK_C, MASK_B, MASK_OP, }
  config.nABC = { "A", "C", "B", "OP", }
  ...
  config.opmode = "0000001000000000000000000101200000000000000000000000000022001000000000000000000000"
...
function DescribeInst(inst, pos, func)
  ...
  -- comments for RK(B), RK(C)
  ---------------------------------------------------------------
  local function CommentBKC(inst, quoteB)
    local B, C = CommentK(inst.B, quoteB), CommentRK(inst.C, true)
    if B == "" then
      if C == "" then return "" else return C end
    elseif C == "" then
      return B
  ...
  ---------------------------------------------------------------
  elseif inst.OP ==  7 then -- MOVE A B
    Operand = OperandAB(inst)
  ---------------------------------------------------------------
  elseif inst.OP ==  25 then -- LOADK A Bx
    Operand = OperandABx(inst)
    Comment = CommentK(inst.Bx, true)
  ---------------------------------------------------------------
  elseif inst.OP ==  13 then -- LOADBOOL A B C
  ...
  ---------------------------------------------------------------
  elseif inst.OP ==  26 then -- LOADNIL A B
    Operand = OperandAB(inst)
  ---------------------------------------------------------------
  elseif inst.OP ==  32 then -- GETUPVAL A B
    Operand = OperandAB(inst)
    Comment = func.upvalues[inst.B + 1]
  ---------------------------------------------------------------
  elseif inst.OP ==  6 or   -- GETGLOBAL A Bx
         inst.OP ==  27 then -- SETGLOBAL A Bx
    Operand = OperandABx(inst)
    Comment = CommentK(inst.Bx)
  ---------------------------------------------------------------
  elseif inst.OP ==  12 then -- GETTABLE A B C
    Operand = OperandABC(inst)
    Comment = CommentRK(inst.C, true)
  ---------------------------------------------------------------
    elseif inst.OP ==  10 then -- GETTABLE_S A B C
    Operand = OperandABC(inst)
    Comment = CommentRK(inst.C, true)
  ---------------------------------------------------------------
  elseif inst.OP ==  33 then -- SETUPVAL A B
    Operand = OperandAB(inst)
    Comment = func.upvalues[inst.B + 1]
  ---------------------------------------------------------------
  elseif inst.OP ==  16 then -- SETTABLE_S A B C
    Operand = OperandABC(inst)
    Comment = CommentRK(inst.C)
  ---------------------------------------------------------------
    elseif inst.OP ==  20 then -- SETTABLE A B C
    Operand = OperandABC(inst)
    Comment = CommentRK(inst.C)
  ---------------------------------------------------------------
  elseif inst.OP == 46 then -- NEWTABLE A B C
    Operand = OperandABC(inst)
    local ar = fb2int(inst.B)  -- array size
    local hs = fb2int(inst.C)  -- hash size
    Comment = "array="..ar..", hash="..hs
  ---------------------------------------------------------------
  elseif inst.OP == 8 then -- SELF A B C
    Operand = OperandABC(inst)
    Comment = CommentRK(inst.C, true)
  ---------------------------------------------------------------
  elseif inst.OP == 34 or   -- ADD A B C
         inst.OP == 36 or   -- SUB A B C
         inst.OP == 38 or   -- MUL A B C
         inst.OP == 40 or   -- DIV A B C
         inst.OP == 42 or   -- MOD A B C
         inst.OP == 44 then -- POW A B C
    Operand = OperandABC(inst)
    Comment = CommentRK(inst.C)
  ---------------------------------------------------------------
  elseif inst.OP == 35 or   -- ADD_BK A B C
         inst.OP == 37 or   -- SUB_BK A B C
         inst.OP == 39 or   -- MUL_BK A B C
         inst.OP == 41 or   -- DIV_BK A B C
         inst.OP == 43 or   -- MOD_BK A B C
         inst.OP == 45 then -- POW_BK A B C
    Operand = OperandABC(inst)
    Comment = CommentBKC(inst,true)
  ---------------------------------------------------------------
  elseif inst.OP == 47 or   -- UNM A B
         inst.OP == 48 or   -- NOT A B
         inst.OP == 49 then -- LEN A B
    Operand = OperandAB(inst)
  ---------------------------------------------------------------
  elseif inst.OP == 54 then -- CONCAT A B C
    Operand = OperandABC(inst)
  ---------------------------------------------------------------
  elseif inst.OP == 28 then -- JMP sBx
    Operand = string.format(config.FORMAT_Bx, inst.sBx)..config.PAD_Bx
    Comment = CommentLoc(inst.sBx)
  ---------------------------------------------------------------
  elseif inst.OP == 4 or    -- EQ A B C
         inst.OP == 50 or   -- LT A B C
         inst.OP == 52 or   -- LE A B C
         inst.OP == 55 then -- TESTSET A B C
    Operand = OperandABC(inst)
    if inst.OP ~= 55 then Comment = CommentRK(inst.C) end
  ...
  ---------------------------------------------------------------
  elseif inst.OP == 5 or    -- EQ_BK A B C
         inst.OP == 51 or   -- LT_BK A B C
         inst.OP == 53 then -- LE_BK A B C
    Operand = OperandABC(inst)
    if inst.OP ~= 55 then Comment = CommentBKC(inst,true) end
    if Comment ~= "" then Comment = Comment..", " end
    -- since the pc++ is in the 'else' path, the sense is opposite
    local sense = " if false"
    if inst.A == 0 then sense = " if true" end
    Comment = Comment..CommentLoc(1, sense)
  ---------------------------------------------------------------
  elseif inst.OP == 1 then -- TEST A C
    Operand = OperandAC(inst)
    local sense = " if false"
    if inst.C == 0 then sense = " if true" end
    Comment = Comment..CommentLoc(1, sense)
  ---------------------------------------------------------------
  elseif inst.OP == 30 or   -- CALL A B C
         inst.OP == 31 then -- TAILCALL A B C
    Operand = OperandABC(inst)
  ---------------------------------------------------------------
  elseif inst.OP == 9 then -- RETURN A B
    Operand = OperandAB(inst)
  ---------------------------------------------------------------
  elseif inst.OP == 57 then -- FORLOOP A sBx
    Operand = OperandAsBx(inst)
    Comment = CommentLoc(inst.sBx, " if loop")
  ---------------------------------------------------------------
  elseif inst.OP == 56 then -- FORPREP A sBx
    Operand = OperandAsBx(inst)
    Comment = CommentLoc(inst.sBx)
  ---------------------------------------------------------------
  elseif inst.OP == 14 then -- TFORLOOP A C
    Operand = OperandAC(inst)
    Comment = CommentLoc(1, " if exit")
  ---------------------------------------------------------------
  elseif inst.OP == 58 then -- SETLIST A B C
    Operand = OperandABC(inst)
  ...
  ---------------------------------------------------------------
  elseif inst.OP == 59 then -- CLOSE A
    Operand = string.format(config.FORMAT_A, inst.A)
  ---------------------------------------------------------------
  elseif inst.OP == 60 then -- CLOSURE A Bx
    Operand = OperandABx(inst)
    -- lets user know how many following instructions are significant
    Comment = func.p[inst.Bx + 1].nups.." upvalues"
  ---------------------------------------------------------------
  elseif inst.OP == 61 then -- VARARG A B
    Operand = OperandAB(inst)
  ---------------------------------------------------------------
  elseif inst.OP ==  0  or      -- GETFIELD A B C
		 inst.OP ==  67  then   -- GETFIELD_R1 A B C
    Operand = OperandABC(inst)
    Comment = CommentK(inst.C)
  ---------------------------------------------------------------
  elseif inst.OP ==  15 or     -- SETFIELD A B C
		 inst.OP ==  68 then   -- SETFIELD_R A B C
    Operand = OperandABC(inst)
    Comment = CommentBKC(inst,false)
  ---------------------------------------------------------------
  else
